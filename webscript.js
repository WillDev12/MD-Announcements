...
$(function() {
    ...
    $('#generator').on("click", function() {
        $('#dataurl').val(`<img src="${$('#outputimg').attr('src')}" style="width: 100%;"></img>`);
    });
    $('#copybtn').on("click", function() {
        navigator.clipboard.writeText($("#dataurl").val());
        document.getElementById("dataurl").select();
        $('#textappend').append(`<p style="color: #41D274;" id="text">Copied to clipboard</p>`);
    })
...
$(function() {
    $('#color1, #color2, #color3, #color4, #color5, #color6, #color7, #color8').on("click", function() {
